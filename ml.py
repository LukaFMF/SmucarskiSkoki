...
def PredictNextJump(athlete,hillSize,country,date,series,points1 = None):
	...
	for res in athlete.personalResults:
		...
		if (len(res) == 3 and
			(result.points1 != None or result.points2 != None) and 
			...
			if series == 1 and result.points1 != None:
				data.append([competition.hillSizeHeight,float(athlete.country == event.country),float(timestamp),result.points1])
			if series == 2 and result.points1 != None and result.points2 != None:
				data.append([result.points1,competition.hillSizeHeight,float(athlete.country == event.country),float(timestamp),result.points2])
	...
	if points1 != None and series == 2:
		return regressor.predict([[points1,float(hillSize),float(athlete.country == country),timestamp]])[0]
	return regressor.predict([[float(hillSize),float(athlete.country == country),timestamp]])[0]
